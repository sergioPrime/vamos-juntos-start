...
SELECT o.id, 'Dinheiro', 'cash' FROM public.organizations o
WHERE NOT EXISTS (
  SELECT 1 FROM public.payment_methods pm 
  WHERE pm.org_id = o.id AND pm.name = 'Dinheiro'
);

INSERT INTO public.payment_methods (org_id, name, type) 
SELECT o.id, 'PIX', 'pix' FROM public.organizations o
WHERE NOT EXISTS (
  SELECT 1 FROM public.payment_methods pm 
  WHERE pm.org_id = o.id AND pm.name = 'PIX'
);

INSERT INTO public.payment_methods (org_id, name, type) 
SELECT o.id, 'Cartão de Débito', 'debit' FROM public.organizations o
WHERE NOT EXISTS (
  SELECT 1 FROM public.payment_methods pm 
  WHERE pm.org_id = o.id AND pm.name = 'Cartão de Débito'
);

INSERT INTO public.payment_methods (org_id, name, type) 
SELECT o.id, 'Cartão de Crédito', 'credit' FROM public.organizations o
WHERE NOT EXISTS (
  SELECT 1 FROM public.payment_methods pm 
  WHERE pm.org_id = o.id AND pm.name = 'Cartão de Crédito'
);
