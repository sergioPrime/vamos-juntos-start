...
-- Add unique constraints for data integrity (with safe checks)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'unique_org_account_code' 
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'unique_org_center_code' 
...
DROP TRIGGER IF EXISTS trigger_validate_chart_of_accounts ON public.chart_of_accounts;
DROP TRIGGER IF EXISTS trigger_validate_cost_centers ON public.cost_centers;
DROP TRIGGER IF EXISTS trigger_validate_financial_entries ON public.financial_entries;
DROP TRIGGER IF EXISTS trigger_chart_of_accounts_updated_at ON public.chart_of_accounts;
DROP TRIGGER IF EXISTS trigger_cost_centers_updated_at ON public.cost_centers;
DROP TRIGGER IF EXISTS trigger_financial_entries_updated_at ON public.financial_entries;
