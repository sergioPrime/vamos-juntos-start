...
ADD CONSTRAINT caixa_movimentacoes_valor_positivo CHECK (valor > 0);
...
DECLARE
  ...
  sessao_org uuid;
BEGIN
  -- Validar se a sessão existe e está aberta
...
CREATE INDEX idx_transaction_audit_org_table 
ON public.transaction_audit(org_id, table_name, created_at DESC);

CREATE INDEX idx_transaction_audit_record 
